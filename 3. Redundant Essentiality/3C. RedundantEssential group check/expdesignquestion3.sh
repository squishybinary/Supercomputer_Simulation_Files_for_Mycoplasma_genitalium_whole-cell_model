#!/bin/bash -login
# Title: expdesignquestion3
# Author: Cameron Matthews, cm15159@bristol.ac.uk
...
#SBATCH --array=1-132 	
# Name of Job
#SBATCH --job-name=expdesignquestion3
# Location of Log Output
#SBATCH --output=/projects/flex1/SummerStudents/Cameron/output/expdesignquestion3/slurm-%A_%a.out

#############################################################################################################################################################

### Declarations
# WholeCell-master directory on BlueGem that contains analysis files
Master=/home/cm15159/WholeCell-master/WholeCell-master

# Directory to contain simulation output
OutDir=/projects/flex1/SummerStudents/Cameron/output/expdesignquestion3

# Variables passed to analysis
# Fetch the Experiment name from the Exp list file
experiment_list=/home/cm15159/BlueGem/ExpLists/expdesignquestion3_exp.list

Experiment=$(awk NR==${SLURM_ARRAY_TASK_ID} ${experiment_list})
Sim=${SLURM_ARRAY_TASK_ID}

# Fetch the Gene Knock Outs from the KO list file
ko_list=/home/cm15159/BlueGem/KOLists/expdesignquestion3_ko.list
...
if [ -f "state-0.mat" ]; then
	...
	matlab $options -r "addpath('${Master}');runGraphs('${Experiment}','${Sim}');compareGraphs('${Experiment}','${Sim}','${BackgroundFig}');exit;"
fi
...
# find /projects/flex1/Cameron/output/expdesignquestion3 -type f -iname "*.pdf" -exec mv -t /projects/flex1/Cameron/output/expdesignquestion3/pdfs {} +
# find /projects/flex1/Cameron/output/expdesignquestion3 -type f -iname "*.fig" -exec mv -t /projects/flex1/Cameron/output/expdesignquestion3/figs {} +
